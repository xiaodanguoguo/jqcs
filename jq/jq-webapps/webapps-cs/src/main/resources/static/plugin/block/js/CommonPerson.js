...
CommonPerson.Base.LoadingPic = {
    operation: {
        ...
        // loadingImgUrl: "${URL_LIB}/static/common/image/block/blue-loading.gif",  //默认load图地址
         loadingImgUrl: "../images/blue-loading.gif",  //默认load图地址
        loadingImgHeight: 24,               //Loading图的高
        loadingImgWidth: 24                 //Loading图的宽
    },

    //显示全屏Loading图
    //@param:sunWin,是否为子窗口，传“y”
    FullScreenShow: function (msg,sunWin) {
        if (msg === undefined) {
            msg = "数据加载中, 请稍等...";
        }
        ...
        if (this.operation.loadingCount < 1) {
            ...
            $("#div_loadingImg").append("<div id='loadingPage_bg' class='loadingPage_bg1'></div><div id='loadingPage'>" + msg + "</div>");
            if(sunWin && sunWin == "y")
                $("#loadingPage_bg").height($(window.parent.document).find("iframe").height()).width($(window.parent.document).find("iframe").width());
            else
                $("#loadingPage_bg").height($(top.window.document).height()).width($(top.window.document).width());
        }
